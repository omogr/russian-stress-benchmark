...
- Есть словарь слов, для которых допыстимы несколько вариантов ударений — такие слова не учитываются.
...
После выполнения скрипта `run.sh` (или `run.bat`) в директории `output/` создаются:
...
Добавление новых библиотек и повторное тестирование с другими данными не представляет большой сложности. Для добавления новой библиотеки надо создать новый акцентуатор в run_accentuator.py и добавить вызовы в `run.sh` по аналогии с silero_stress.
